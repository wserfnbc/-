...
class WechatService extends Service
{
     ...
     * @return array
     */
    public static function buildAuthData(array $info)
    {
    ...
    public function getWechatConfig($authorizerAppid)
    {
        ...
        $conifg['GetAccessTokenCallback'] = function ($authorizerAppid) {
            ...
            $refreshToken = $this->app->db->name('WechatConfig')->where($map)->value('authorizer_refresh_token');
            ...
            // 更新公众号授权信息
            $this->app->db->name('WechatConfig')->where($map)->update([
    ...
    {
        return $this->app->getRootPath() . 'runtime/service/wechat';
    }
}